...
def _collapse_subproperty_chain(g, rep):
    ...
    i = 0
    changed = True
    while changed and i < MAX_ITERATIONS:
        changed = False
        i += 1
        # ----------------------------------------------------------
        ...
        if i % 20 == 1:            # print every 20th iteration
            ...
            print(f"[subChain] {rep}  iteration {i}  dangling subprops: {dangling}")
        
        # Collect subjects first to avoid modifying graph while iterating
        subjects_to_process = list(g.subjects(RDFS.subPropertyOf, rep))
        
        for sub in subjects_to_process:
            # Check if edge still exists (may have been removed in previous iteration)
            if (sub, RDFS.subPropertyOf, rep) not in g:
                continue
            ...
            if (rep, RDFS.subPropertyOf, sub) in g:            # scm-eqp2
                ...
                for p3 in g.subjects(RDFS.subPropertyOf, sub):  # scm-spo
                    if rep != p3 and (p3, RDFS.subPropertyOf, rep) not in g:
    ...
    if i >= MAX_ITERATIONS:
        print(f"WARNING: _collapse_subproperty_chain hit MAX_ITERATIONS for {rep}")
...
def _collapse_equivalent_properties(g, rep, prop_shapes, sg, path_set):
    MAX_ITERATIONS = 1000
    j = 0
    while _needs_property_merge(g, rep) and j < MAX_ITERATIONS:
        j += 1
        # ----------------------------------------------------------
        if j % 20 == 1:
        ...
        equiv_subjects = list(g.subjects(OWL.equivalentProperty, rep))
        equiv_objects = list(g.objects(rep, OWL.equivalentProperty))
        
        for p in equiv_subjects:
            g.remove((p, OWL.equivalentProperty, rep))
            if p != rep:
                g.add((rep, OWL.sameAs, p))
                
        for p in equiv_objects:
            g.remove((rep, OWL.equivalentProperty, p))
            if p != rep:
                g.add((rep, OWL.sameAs, p))

        # ---- normalise incoming sameAs direction
        same_subjects = list(g.subjects(OWL.sameAs, rep))
        for p in same_subjects:
            if p != rep:
                g.remove((p, OWL.sameAs, rep))
                g.add((rep, OWL.sameAs, p))

        # ---- merge every sameAs peer into rep
        same_objects = list(g.objects(rep, OWL.sameAs))
        for same_p in same_objects:
            g.remove((same_p, OWL.sameAs, same_p))             # drop reflexive

            if same_p != rep:
                # copy predicate/object triples
                for pred, obj in list(g.predicate_objects(same_p)):
                    g.remove((same_p, pred, obj))
                    if pred != OWL.sameAs:  # Don't create new sameAs loops
                        g.add((rep, pred, obj))

                # copy subject/predicate triples
                for subj, pred in list(g.subject_predicates(same_p)):
                    g.remove((subj, pred, same_p))
                    if pred != OWL.sameAs:  # Don't create new sameAs loops
                        g.add((subj, pred, rep))

                # rewrite (subj same_p obj) → (subj rep obj)
                for subj, obj in list(g.subject_objects(same_p)):
    ...
    if j >= MAX_ITERATIONS:
        print(f"WARNING: _collapse_equivalent_properties hit MAX_ITERATIONS for {rep}")
